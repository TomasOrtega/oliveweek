...
Install test-only dependencies with: pip install playwright==1.55.0
Then: python -m playwright install chromium && npm run test:browser
"""
import json
import os
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            try:
                ...
                page.wait_for_function("[...document.querySelectorAll('.meal img')].every(i=>i.complete&&i.naturalWidth>0)")
                ...
                expect(page.get_by_role('button', name='Generate plan', exact=True)).to_be_enabled()
                locked = state(page)['plan']['days'][0]['meals'][0]
                check('locked recipe and portion survive regeneration', locked['recipeId'] == first['recipeId'] and locked['servings'] == first['servings'] and locked['locked'])
                page.locator('.meal').first.locator('[data-action="lock"]').click()
                page.locator('input[data-change="portion"]').first.fill('1.25')
                page.locator('input[data-change="portion"]').first.press('Tab')
                check('portion changes update persisted state', state(page)['plan']['days'][0]['meals'][0]['servings'] == 1.25)

                page.locator('.meal').nth(1).locator('[data-action="swap"]').click()
                page.locator('[data-action="choose-swap"]').first.click()
                lunches = [next(m for m in day['meals'] if m['slot']=='lunch') for day in state(page)['plan']['days']]
                check('swap replaces repeated lunches together', len({m['recipeId'] for m in lunches}) == 1)
                old = state(page)
                ...
                check('recipe dialog has a photograph, weighed ingredients and source credit', page.locator('.ingredients li').count() > 0 and page.get_by_role('link', name='Photo source').count()==1)
                ...
                check('shopping checkboxes persist', any(state(page)['checked'].values()))
                with page.expect_download() as dl:
                    page.get_by_role('button', name='CSV', exact=True).click()
                dl.value.save_as(OUT/'shopping.csv')
                check('CSV export contains aggregated weights', 'Buy (g)' in (OUT/'shopping.csv').read_text())
                ...
                page.locator('#dialog input[name="name"]').fill('Test lentils from label')
                page.locator('#dialog input[name="basis"]').fill('dry weight')
                for key, value in {'kcal':'350', 'protein':'25', 'carbs':'60', 'fat':'1', 'fiber':'10', 'sodium':'5', 'saturatedFat':'0.1'}.items():
                    page.locator(f'#dialog input[name="{key}"]').fill(value)
                page.get_by_role('button', name='Save ingredient', exact=True).click()
                check('label-based custom ingredients are saved', len(state(page)['customFoods']) == 1)
                ...
                page.locator('#dialog input[name="name"]').fill('My photographed recipe')
                page.get_by_role('button', name='Save recipe copy').click()
                check('custom recipe retains credited original photograph', len(state(page)['customRecipes']) == 1 and bool(state(page)['customRecipes'][0]['photo']))
                ...
                expect(page.locator('#dialog')).to_contain_text('No calorie or macro values are inferred')
                check('unmapped source recipes never invent nutritional totals', page.locator('#dialog .inline-nutrients').count() == 0)
                close_dialog(page)
                ...
                check('backup export includes recipes, pantry and saved plans', exported == state(page))
                before = state(page)
                page.locator('#backup-file').set_input_files({'name':'invalid.json','mimeType':'application/json','buffer':b'{invalid'})
                expect(page.locator('#toast')).to_contain_text('not valid JSON')
                check('invalid backup does not overwrite existing data', state(page) == before)
                ...
                check('preference changes generate a fresh plan', state(page)['preferences']['diet'] == 'vegan')

                page.locator('.header nav a[href="#prep"]').click()
                expect(page.locator('.prep-job')).not_to_have_count(0)
                check('prep view shows batch quantities and storage guidance', 'within 3 days' in page.locator('#main').inner_text())
                page.get_by_role('button', name='Batch quantities').first.click()
                check('prep job opens its scaled recipe', float(page.locator('#recipe-servings').input_value()) > 0)
                close_dialog(page)

                for width in (390, 320):
                    page.set_viewport_size({'width':width,'height':844})
                    for route in ('plan','recipes','shopping','prep','pantry','saved'):
                        page.goto(BASE+'#'+route, wait_until='networkidle')
                        page.wait_for_selector('#main h1')
                        check(f'{route} has no page overflow at {width}px', page.evaluate('document.documentElement.scrollWidth <= innerWidth+1'))
                    if width==390:
                ...
                page.reload(wait_until='networkidle')
                expect(page.locator('.meal')).to_have_count(4)
                check('offline reload loads the app and saved plan', state(page) == await_state)
                page.locator('.header nav a[href="#shopping"]').click()
                expect(page.locator('.grocery-item')).not_to_have_count(0)
                check('offline grocery list works')
                context.set_offline(False)
                check('no third-party runtime requests', all(u.startswith('http://127.0.0.1:4173/') for u in requests))
                ...
                (OUT/'browser-report.json').write_text(json.dumps({'passed':len(checks),'checks':checks,'javascriptErrors':errors},indent=2))
                browser.close()
    finally:
        server.terminate()
