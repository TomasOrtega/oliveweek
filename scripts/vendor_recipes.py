...
def field(front, key, default=''):
    m = re.search(r'^' + key + r':\s*(.*?)\s*$', front, re.M)
...
def main():
    ...
    request = urllib.request.Request(url, headers={'User-Agent': 'OliveWeek-recipe-vendor/2.0'})
    ...
    out, photos, upstream = ROOT / 'data', ROOT / 'assets/recipes', ROOT / 'vendor/based-cooking'
    ...
    for name in sorted(names):
        ...
        steps = items(section(body, ['Directions', 'Instructions', 'Preparation', 'Method']), True)
        ...
        tags = []
        raw_tags = re.search(r'^tags:\s*\[(.*?)\]', front, re.M)
        if raw_tags:
        ...
        if author_path in names:
            try:
                ...
                pass
        records.append({
            'id': 'bc-' + slug, 'slug': slug, 'name': title, 'author': author,
            'tags': tags, 'ingredientsText': ingredients, 'steps': steps,
            'notes': plain(section(body, ['Notes'])), 'photo': photo,
            ...
            'license': 'Public domain (Unlicense)', 'revision': REVISION,
    ...
    (out / 'community.json').write_text(json.dumps(records, ensure_ascii=False, indent=2) + '\n')
    ...
    (upstream / 'PROVENANCE.json').write_text(json.dumps({
        ...
        'excluded': EXCLUDED, 'count': len(records)
    }, indent=2) + '\n')
    print(f'Vendored {len(records)} recipes and their original photographs at {REVISION}')
